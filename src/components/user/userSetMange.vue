<template>
  <div class="flex">
    ...
    <div class="border-b-2 p-1 border-primaryLight w-full text-end">
      <button class="btn mr-2" @click="newSet" :disabled="settingToggle">
        新增+
      </button>
      <button class="btn" @click="updateSetOpen" :disabled="newSetToggle">
        開啟編輯
      </button>
    </div>
  </div>
  <div class="w-full border-x-2 border-b-2 border-primaryLight bg-background">
    <div class="flex justify-between">
      <div class="flex gap-2 p-2 items-center">
        <div v-if="!newSetToggle">
          <span class="font-bold">代號:</span>
          <label class="inpLabel">
            <span class="inp">{{ currentTab?.userset }}</span></label
          >
        </div>
        <div v-if="!newSetToggle">
          <span class="font-bold">名稱:</span>

          <label v-if="!settingToggle" class="inpLabel">
            <span class="inp">{{ currentTab?.name }}</span>
          </label>
        ...
        <div v-else>
          ...
          <span class="font-bold">名稱:</span>
          <label class="inpLabel">
            <input class="inp" v-model="setUpdate.name" />
          </label>
        </div>
      </div>
      ...
      <div v-if="settingToggle" class="flex items-center p-1 gap-2">
        ...
        <button class="btn bg-cancel" @click="settingToggle = !settingToggle">
          取消
        </button>
      </div>
    </div>
    <button class="btn my-2" @click="authToggle = !authToggle">權限列表</button>
...
<script setup>
import { onBeforeMount, ref, watch, readonly, markRaw, nextTick } from "vue";
...
watch(currentTab, () => {
  for (let item of userStore().svcRoleSet) {
    if (currentTab.value.userset == item.userset) {
